...
namespace DBTest
{
    ...
    class CommandLineParser
    {
        ...
        public string GetArgOrBlank(string name, string defaultValue = "")
        {
            ArrayList values = null;
            values = GetArgs(name);
            if (values.Count == 0) return defaultValue;
            if (values.Count == 1) return ((CommandLineArgs)values[0]).value;
            throw new System.InvalidOperationException($"Argument '{name}' appears more than once.");
        }
